...
**Periodic construction and physical-feedback learning for crystal generation.**

CrystalDLM connects periodic geometric modeling with learning from measured structural outcomes. C1 couples the DLM's coordinate candidates through lattice-conditioned periodic interactions. Frozen continuous diffusion turns the resulting draft into a continuous reference. C2 converts physically evaluated reference/candidate outcomes into conditional DLM supervision and learned relative verification, so that physical experience can be reused in later reconstruction decisions.
...
This documentation is on **`codex/verified-feedback-train-20260920`**. The overview explains the retained conditional reconstruction method (`dlm train c2`, `dlm run`) through its existing physical-feedback training. This update changes the narrative, with the main experiment's data, model assets, execution and evaluation definitions held fixed. The branch also contains a [separate registered draft-feedback recipe](#registered-draft-feedback-a-separate-recipe), with its own parameter path and evidence.
...
## Registered draft feedback: a separate recipe

This branch additionally contains `python -m dlm_iclr.feedback`. That recipe uses registered complete teachers and physically checked actual-prefix supervision to update the **draft DLM**, with C1 parameters fixed. It is distinct from the conditional editor/value training above.

[Registered method and mathematics](docs/C1_C2_METHOD_ZH.md) · [Short explanation](docs/C2_FEEDBACK_ZH.md) · [Material contract and commands](docs/registered-feedback.md)

Its small controlled seen-TRAIN study improved the native raw stability proxy from 7/64 to 35/64 at matched temperature, with concentration and F800 tradeoffs. The expanded 1,306-request raw comparison did not reproduce that native gain: 1/1,306 in both arms; conventional SUN 33→34 and MSUN 178→159, with worsening mean native energy and force RMS. These are separate recipe results, not results of a newly reframed KEEP/EDIT model. [Small-study scope](docs/results/registered-feedback-train.md) · [Expanded raw report](docs/results/reporting-1306-raw.md) · [Numeric snapshot](docs/results/reporting-1306-raw.json).

F800/F400 paired evaluation is incomplete and research execution remains paused for discussion. This documentation update starts no new training or evaluation. The registered recipe's assets, budget ledger and evidence retain their original bindings.
