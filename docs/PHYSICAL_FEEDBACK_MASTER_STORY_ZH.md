...
本文集中说明科学动机与学习机制；运行配置和历史结果的复现条件见[实现与复现说明](https://github.com/YWDDLiang/DLM_ICLR/blob/main/docs/keep-edit-implementation.md)。

本文以 main 的既有 KEEP/EDIT 方法和保存的主实验为基础，组织科学动机、方法与数学。摘要采用现有 3000 请求面板的原配置结果，不重新计算或更改指标。首次生成器的反馈更新与减少部署精修依赖作为后续方向，和已经实现的条件重构学习分开说明。

## Abstract

Crystal generation requires the coordinated prediction of chemical compositions, lattice geometries, and periodic atomic coordinates. While language–diffusion hybrids approach this task through discrete drafting followed by continuous refinement, two challenges remain: capturing periodic dependencies during drafting and converting refinement experience into reusable knowledge for discrete conditional generation.

To address these challenges, we propose CrystalDLM, a periodic diffusion language modeling framework with physical-feedback learning. Its bidirectional masked prediction conditions unresolved geometric fields on visible context, providing a shared interface for structured construction and learning from physically verified targets. First, CrystalDLM augments site-wise coordinate scores with lattice-conditioned periodic interaction potentials, producing a tractable joint distribution over unresolved positions. Second, a continuous-to-discrete feedback mechanism uses frozen diffusion to establish continuous references from self-generated drafts. Conditional DLM proposals are represented in crystal tokens, executed as patches that preserve unchanged continuous fields, and evaluated through interatomic-potential relaxation to construct geometric supervision and within-condition outcome comparisons. Masked reconstruction, reference regularization, and a reward-weighted physical teacher train the conditional DLM, while a relative verifier learns which proposals should replace the reference. Measured structural outcomes thus inform subsequent conditional generation and selection through learned parameters.

On the retained, historically selected 3,000-request MP-20 panel, the existing C2 configuration achieves confirmed SUN and MSUN rates of 9.87% and 49.00%, respectively, compared with 8.17% and 46.00% for F800. By connecting discrete periodic modeling with continuous physical evaluation, CrystalDLM amortizes physical experience into reusable conditional reconstruction and verification, advancing physically informed crystal generation.

## 中文摘要

晶体生成需要协调预测化学组成、晶格几何和周期原子坐标。语言—diffusion 混合方法通过离散 draft 与连续精修结合两种表示，但仍面临两个问题：怎样在 draft 构造中显式表达周期关系，以及怎样将精修经验转化为离散条件生成可以复用的知识。

为此，我们提出结合物理反馈学习的周期扩散语言建模框架 CrystalDLM。双向 masked prediction 在可见上下文下预测待定几何字段，为结构化构造和核验目标学习提供统一接口。首先，C1 在逐位置坐标评分上加入晶格条件化的周期相互作用，形成可计算的联合候选分布。其次，连续到离散的反馈机制利用冻结 diffusion，将自生成 draft 转为连续结构参照。条件 DLM 的提案以晶体 token 表示，通过保留未变连续字段的 patch 执行，再经原子间势弛豫与评价形成几何监督和同条件后果比较。重构、参考约束与奖励加权物理教师共同训练条件 DLM，相对验证模型学习哪些提案值得替代原参照，从而使已测物理经验进入后续条件生成与选择的参数。

在保留的历史选定 3000 请求 MP-20 面板上，原 C2 配置的已确认 SUN/MSUN 为 9.87%/49.00%，对应 F800 为 8.17%/46.00%。CrystalDLM 将离散周期建模与连续物理评价连接起来，把物理经验转化为可复用的条件重构与验证能力。

结果与完整请求分母见[原 3000 面板报告](https://github.com/YWDDLiang/DLM_ICLR/blob/main/docs/results/C2_HISTORY1000_TOP2_3000/RESULT_ZH.md)，摘要使用原 `no_rollback` 配置的已确认比例。
...
本节定义解释已有证据需要回答的科学问题，不新增或调整主实验。使用保存的 Plan、参照、候选、结构身份和标签核查；已有记录不能回答的问题作为证据边界保留。
...
在保存的历史选定 3000 请求 MP-20 面板上，原 C2 配置的已确认 SUN/MSUN 为 9.87%/49.00%，F800 为 8.17%/46.00%，均以完整请求数为分母。完整结果保留未知、来源选择和运行条件，分别解释候选质量、选择质量与额外计算的作用。方法将连续结构计算产生的经验转化为可复用的条件模型参数；当前反馈落在条件重构阶段，将其继续前移到首次 raw 生成器属于后续方向。
...
实现依据为 GitHub `main` 的主实验基线 `2e78750dc4ca9a9bfce0b7cef7eda1d657e24402`。此次更新仅包含文档，保留该版本的科学代码、配置、数据选择和结果。
...
# Appendix D. main 主实验与摘要结果绑定

本文使用 main 已保存的实验，不引入新 Plan、重训练或重评价。主要面板的定义为：

| 面板 | 已有数据定义 | 报告范围 |
|---|---|---|
| 历史 1050 | 同一组原请求；统一表另列 1005 条共同已知子集 | 子集和完整请求分母分开，不能互换 |
| 历史最佳 3000 | 原历史 1000 加两组各 1000 条已选历史块 | 按保存的完整面板顺序重算 U/N；摘要采用此面板 |
| 历史扩展 5000 | 原有扩展面板与已保存选择清单 | 独立报告，不与其他面板相加 |

摘要使用 3000 面板中原 `no_rollback` 配置，与 F800 在相同请求分母下比较：

| 原结果 | SUN | SUN 未知 | MSUN | MSUN 未知 |
|---|---:|---:|---:|---:|
| F800 | 245/3000，8.17% | 68 | 1380/3000，46.00% | 68 |
| 原 C2 配置 | 296/3000，9.87% | 92 | 1470/3000，49.00% | 92 |

这是历史选定面板上的已确认比例，失败与未知仍在分母；不改称未选择的独立测试。运行规则另见实现文档，其他已报告配置继续按原结果表保留。

固定来源：[结果 JSON](https://github.com/YWDDLiang/DLM_ICLR/blob/main/docs/results/C2_HISTORY1000_TOP2_3000/RESULT.json)、[完整报告](https://github.com/YWDDLiang/DLM_ICLR/blob/main/docs/results/C2_HISTORY1000_TOP2_3000/RESULT_ZH.md)、[逐请求选择清单](https://github.com/YWDDLiang/DLM_ICLR/blob/main/docs/results/C2_HISTORY1000_TOP2_3000/selection_manifest.json)。
