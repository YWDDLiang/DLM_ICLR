...
The three runs use the already saved H1A2 Plans; no new Planner sampling is performed. Run 1 retains their saved G/F sampling streams. Runs 2 and 3 derive new G/F streams with seeds 314159 and 271828, while preserving every Plan's identity, conditions, prompt and original order. Each run then keeps its streams fixed across S0-S3. Training seeds are 20260911, 20261911 and 20262911, incremented by the round index for each update. The protocol and source digests were recorded before formal generation in [formal_protocol.json](formal_protocol.json).
