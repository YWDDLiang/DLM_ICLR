...
def dynamic_answer_token_count(num_atoms: int) -> int:
    ...
    if not 1 <= value <= FixedSlotConfig().max_atoms:
        raise FixedSlotError(f"Atom count {value} outside 1..{FixedSlotConfig().max_atoms}")
    return 7 + 4 * value
...
def parse_dynamic_answer(
    ...
    if strict:
        ...
        if residual.strip():
            raise FixedSlotError(f"Strict dynamic answer contains non-schema text {residual.strip()[:40]!r}")
