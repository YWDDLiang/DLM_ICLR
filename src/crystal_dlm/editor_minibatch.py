...
from crystal_dlm.rsi_minibatch import (
...
def train_editor_minibatches(
    model, tokenizer, examples, spec, selected, reference, optimizer, support, output, write_json
):
    ...
    for epoch, indices in epoch_indices(
        len(examples), batch_size=spec["batch_size"], world=world, epochs=spec["epochs"], seed=spec["seed"]
    ):
