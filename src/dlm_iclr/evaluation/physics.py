...
import time
import numpy as np
from dlm_iclr._core.terminal_energy_consistency import (
...
from dlm_iclr.runtime.io import file_hash, fingerprint, read_json, write_json
...
def physical_identity(checkpoint, protocol=None):
    return {
        "checkpoint_sha256": file_hash(checkpoint),
        "implementation_sha256": {
            name: file_hash(Path(__file__).with_name(name)) for name in ("physics.py", "relaxation.py")
        },
        "protocol": {**RELAXATION_PROTOCOL, **(protocol or {})},
...
class Labeler:
    def __init__(self, checkpoint, device="cuda:0", protocol=None):
        import torch
        from ase.optimize import FIRE
        from ase.filters import FrechetCellFilter
        from chgnet.model.model import CHGNet
        from chgnet.model.dynamics import StructOptimizer
        import dlm_iclr.evaluation.relaxation as physics_core
    ...
    def label(self, record):
        result = self.core.label_record(
            ...
            fmax=self.protocol["fmax"],
            stress_tolerance=self.protocol["stress_tolerance_GPa"],
...
def _worker(connection, checkpoint, device, cache, protocol=None):
    labeler = Labeler(checkpoint, device, protocol)
...
def label_records(
    ...
    *,
    ...
    identity = physical_identity(checkpoint, protocol)
    ...
    arguments = [
        (str(checkpoint), device, str(cache), protocol)
