...
def score_records(
    ...
    *,
    ...
    reference = (
        hull_reference
        if hull_reference is not None
        else _reference(str(reference_path.resolve()), file_hash(reference_path))
    )
    ...
    if all_predicates:
        required = list(range(len(structures)))
    novel, unique, nu = [None] * len(structures), [None] * len(structures), None
    if required:
        train, index, identity = (
            training_index
            if training_index is not None
            else load_training_index(novelty_reference, cache=cache / "training_index")
        )
    ...
    for i, (record, label) in enumerate(zip(records, labels, strict=True)):
        ...
        if index is not None:
            from dlm_iclr._vendor.crysllmgen.validity import smact_validity, structure_validity
...
def evaluate_sun(
    ...
    *,
    ...
    """
    from dlm_iclr.evaluation.inputs import load_records
    ...
    if labels_path is not None:
        ...
        from dlm_iclr.evaluation.physics import label_records
        from dlm_iclr.runtime.device import setup_device
