...
def get_max_neighbors_mask(natoms, index, atom_distance, max_num_neighbors_threshold):
    ...
    index_sort_map = (
        index * max_num_neighbors + torch.arange(len(index), device=device) - index_neighbor_offset_expand
    )
    ...
    distance_real_cutoff = (
        distance_sort[:, max_num_neighbors_threshold].reshape(-1, 1).expand(-1, max_num_neighbors) + 0.01
    )
