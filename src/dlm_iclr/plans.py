...
from crystal_dlm.fixed_slot import SYMBOL_TO_Z, FixedSlotConfig
from crystal_dlm.r5_plan_state import build_body_prompt
from .io import fingerprint, read_rows, write_json, write_rows

PRESETS = ("H1A2_1200", "R03_256")
...
def load_plans(source, *, requests=None, legal_only=False, seed=17):
    source = str(source)
    path = Path(__file__).parent / "data" / PRESET_FILES[source] if source in PRESET_FILES else Path(source)
    rows = read_rows(path)
...
def prepare_training(source, output, *, exclusions=PRESETS, limit=None):
    """Select sources before generation, excluding every evaluation composition."""
    rows, _ = load_plans(source, legal_only=True)
    forbidden = set()
    for excluded in exclusions:
        other, _ = load_plans(excluded, legal_only=True)
        forbidden.update(composition_key(row["plan_state"]) for row in other)
    selected, omitted, seen = [], [], set()
    for row in rows:
        key = composition_key(row["plan_state"])
        provenance = dict(row.get("provenance", {}))
        role = provenance.get("usage_role", row.get("source_split"))
        if role in ("evaluation", "eval", "val", "validation", "dev", "test"):
            reason = "source_reserved_for_evaluation"
        elif key in forbidden:
            reason = "evaluation_composition"
        elif key in seen:
            reason = "duplicate_composition"
        else:
            reason = None
        if reason:
            omitted.append({"source_id": row["source_id"], "composition": key, "reason": reason})
            continue
        seen.add(key)
        provenance["usage_role"] = "train"
        row = dict(row, provenance=provenance, ordinal=len(selected))
        selected.append(row)
    if limit is not None:
        if len(selected) < limit:
            raise ValueError(f"Only {len(selected)} clean compositions available for requested {limit}")
    ...
    report = {
        ...
        "selected_compositions": len(selected),
        "exclusion_sources": list(map(str, exclusions)),
        "omitted": omitted,
        "selection_uses_physical_outcomes": False,
        "plans_sha256": fingerprint(selected),
    }
