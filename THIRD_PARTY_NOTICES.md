...
The vendored refinement components in `src/dlm_iclr/_vendor/crysllmgen` are derived from [CrysLLMGen](https://github.com/kdmsit/crysllmgen), copyright 2025 Kishalay Das (KD), under the MIT license. The retained license is included both beside that code and in [licenses/CrysLLMGen.txt](licenses/CrysLLMGen.txt). Only the numerical dependency closure needed by the published refiner and validity metrics is included; package imports were adjusted for namespacing.
